...
/*   Updated: 2022/02/10 11:46:09 by ggilbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

// void	init_ray_step_length(t_base *base)
...
void	raycaster(t_base *base)
{
	...
	while (pixx < (int)base->params->res_x)
	{
		...
		if (ray.side == 0)
		{
			wallX = base->player->pos_y + ray.perp_wall_dist * ray.ray_dir_y;
		}
		else
			wallX = base->player->pos_x + ray.perp_wall_dist * ray.ray_dir_x;
		wallX -= floor(wallX);
		if (wallX != 0)
			wallX = fabs(wallX - 1);
		//printf("wallX %f\n", wallX);
